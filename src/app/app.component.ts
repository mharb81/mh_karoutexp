...
export class AppComponent {
  ...
  getStatusBarColor() {
    ...
    rgb2 = headerColor.replace(/\s/g, '').match(/^rgba?\((\d+),(\d+),(\d+)/i) as any;
    ...
    let rgb = color;
    rgb = color.replace(/\s/g, '').match(/^rgba?\((\d+),(\d+),(\d+)/i) as any;
  ...
  setupDeeplinks() {
    this.deeplinks.route({
      ...
      match => {
        if (match.$args && match.$args['slug']) {
          if (match.$route === 'product-detail') {
            this.zone.run(() => {
              this.getSingleProductDetail(match.$args['slug']);
            });
          }

          if (match.$route === 'products') {
            this.zone.run(() => {
              this.getSingleCategoryDetail(match.$args['slug']);
            });
          }
        } else {
          if (match.$route === 'about-us') {
            this.zone.run(() => {
              this.loading.show();
              setTimeout(() => {
                this.loading.hide();
  ...
  getSingleCategoryDetail(slug) {
    this.loading.show();
    const shopPage = this.shared.categories.filter(cat => {
      return cat.slug === slug;
    });
    setTimeout(() => {
      this.navCtrl.navigateForward(`/products/${shopPage[0].id}/${shopPage[0].name}/newest`);
    }, 1500);
  }
  ...
  getSingleProductDetail(slug) {
    ...
    this.config.getWoo("products/" + "?" + this.config.productsArguments + "&slug=" + slug).then((data: any) => {
      this.loading.hide();
      let p = data;
      this.shared.singleProductPageData.push(p[0]);
      this.navCtrl.navigateForward("/product-detail/" + p[0].id);
    }, err => {
      this.loading.hide();
      console.log(err);
    });
