...
export class ConfigService {
  ...
  public url: string = 'https://karoutexpress.com';
  public consumerKey: string = 'ck_f7e4df37669662ce5989846b422f38a6ad16f470';
  public consumerSecret: string = 'cs_efff887ceff50bd798537fafb26ec2bfd3d25853';
  ...
  defaultSettings() {
    ...
    if (this.appNavigationTabs) {
      if (this.homePage != 1) this.currentRoute = "home" + this.homePage;
    }
  ...
  getAbreviation(firstname, lastname) {
    const fname = new String(firstname);
    const lname = new String(lastname);
    const fn = fname.toUpperCase();
    const ln = lname.toUpperCase();
  ...
  refreshToken() {
    this.platform.ready().then(() => {
      this.storage.get('signin_method').then(method => {
        if (method && method === 'apple') {
          this.storage.get('apple_signin_cred').then(authData => {
            if (authData && authData !== null && authData !== undefined && authData.refreshToken !== '' && authData.refreshToken !== undefined && authData.refreshToken !== null) {
              this.http.get(`https://karoutexpress.com/wp-content/harb/mh_auth_verify.php?request_type=apple_refresh&userabrv=MH`, {})
              .pipe(retry(2))
              .subscribe((resData: any) => {
                ...
                this.httpNative.sendRequest('https://appleid.apple.com/auth/token', {method: 'post', data: postData, params: {}, serializer: 'urlencoded', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }}).then(dataRes => {
      
                  const response = JSON.parse(dataRes.data);
                  const username = this.getAbreviation(authData.first_name, authData.last_name);
                  this.http.get(`https://karoutexpress.com/wp-content/harb/mh_auth_verify.php?request_type=apple_authenticate&userabrv=MH&token=${response.id_token}`, {})
                  .pipe(retry(2))
                  .subscribe((resData: any) => {
                    const AppleUserData = {
                      refreshToken: resData.refresh_token
                    };
                    this.storage.set('apple_signin_cred', AppleUserData);
                    setTimeout(() => {
                      this.refreshToken();
                    }, 3600000 - 3000);
                  }, err => {
              ...
              // r44022fd843a947d9b20c42503b01af17.0.mvu.W1cngKgMWhqc577BywWVqg
            }
          });
        }
      });
